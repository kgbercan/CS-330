...
// converts a second format to min:sec:msec
func secToSplit (sec : Double) -> (min:Int, sec:Int, msec:Int){
    let m = sec/60
    let s = (m - floor(m))*60
    let ms = (s - floor(s))*60
    return(Int(m),Int(s),Int(ms))
}
...
let paceGuide: [Int:[String:String]] = [95:ninetyfive, 90:ninety, 85:eightyfive, 80:eighty, 75:seventyfive, 70:seventy, 65:sixtyfive, 60:sixty, 55:fiftyfive]

func pace(min: Int, max: Int, test: String){
    if(min==max){
        print(paceGuide[min]![test]!)
    }
    else{
        var l = Int(min)
        let h = Int(max)
        while(l<=h){
            print("\(l)%: \(paceGuide[l]![test]!)")
            l+=5
        }
    }
}
